...
def _form_payload(html: str) -> tuple[dict[str, str], BeautifulSoup]:
    """Harvest the current PeopleSoft form state for a full-page ICAction POST."""
    soup = BeautifulSoup(html, "html.parser")
    form = soup.find("form", attrs={"name": "win0"}) or soup.find("form")
    if form is None:
        raise RuntimeError("Could not find the PeopleSoft search form")
    ...
    for node in form.find_all("input"):
        ...
        if input_type in {"checkbox", "radio"} and not node.has_attr("checked"):
            continue
        payload[name] = str(node.get("value", ""))
    ...
    for select in form.find_all("select"):
        ...
        payload[name] = str(option.get("value", "")) if option else ""
...
def _set_field_by_prefix(payload: dict[str, str], prefix: str, value: str) -> bool:
    for name in payload:
...
def _class_type_field_name(soup: BeautifulSoup) -> str:
    """Find the dynamic Class Search Type select by its live option values."""
    for select in soup.find_all("select"):
        values = {str(option.get("value", "")) for option in select.find_all("option")}
        if {"OER", "LCT"}.issubset(values):
            return str(select.get("name", ""))
    return ""


def _result_class_numbers(html: str) -> set[str]:
    """Read class numbers from the PeopleSoft MTG_CLASS_NBR result fields."""
    soup = BeautifulSoup(html, "html.parser")
    numbers: set[str] = set()
    for node in soup.find_all(True):
        field_id = str(node.get("id", ""))
        field_name = str(node.get("name", ""))
        if not (field_id.startswith("MTG_CLASS_NBR$") or field_name.startswith("MTG_CLASS_NBR$")):
            continue
        match = CLASS_RE.search(clean(node.get_text(" ", strip=True)) or str(node.get("value", "")))
        if match:
            numbers.add(match.group(1))
    return numbers


def _post_people_soft(session: requests.Session, payload: dict[str, str], attempts: int = 3) -> str:
    last_error: Exception | None = None
    for attempt in range(attempts):
        try:
            response = session.post(BASE_URL, data=payload, timeout=45)
            response.raise_for_status()
            text = response.text
            if "You must have cookies enabled" in text:
                session.get(BASE_URL, timeout=30)
            ...
            if attempt + 1 < attempts:
                time.sleep(2**attempt)
    raise RuntimeError(f"Could not submit PeopleSoft class-type search: {last_error}")


def fetch_class_type_numbers(
    session: requests.Session,
    term_id: str,
    subject_code: str,
    class_type_code: str,
) -> set[str]:
    """Return class numbers matching one public LACCD Class Search Type.

    The public labels are backed by PeopleSoft attribute values. In particular,
    ``OER`` is the internal value for public type 06, Zero Textbook Cost (ZTC),
    and ``LCT`` is public type 07, Low Textbook Cost.
    """
    entry_html = fetch_html(session, BASE_URL)
    payload, soup = _form_payload(entry_html)

    if not _set_field_by_prefix(payload, "SSR_CLSRCH_WRK_CAMPUS$", CAMPUS_CODE):
        raise RuntimeError("Could not locate the PeopleSoft campus field")
    if not _set_field_by_prefix(payload, "CLASS_SRCH_WRK2_STRM$", term_id):
        raise RuntimeError("Could not locate the PeopleSoft term field")
    if not _set_field_by_prefix(payload, "SSR_CLSRCH_WRK_SUBJECT$", subject_code):
        raise RuntimeError("Could not locate the PeopleSoft subject field")

    class_type_field = _class_type_field_name(soup)
    if not class_type_field:
        raise RuntimeError("Could not locate the PeopleSoft Class Search Type field")
    payload[class_type_field] = class_type_code

    # Full-page PeopleSoft search action. ICBcDomData is required for the
    # classic Campus Solutions form to actually execute the action.
    payload["ICAction"] = "CLASS_SRCH_WRK2_SSR_PB_CLASS_SRCH"
    payload["ICBcDomData"] = "UnknownValue"
    payload["ICElementNum"] = "0"
    payload["ICType"] = "Panel"
    payload["ICResubmit"] = "0"
    payload.pop("ICAJAX", None)

    result_html = _post_people_soft(session, payload)
    return _result_class_numbers(result_html)


def enrich_material_flags(
    session: requests.Session,
    term_id: str,
    term_label: str,
    sections: dict[str, dict],
) -> None:
    """Join ZTC/LCT search results back onto the already-scraped class rows."""
    ztc: set[str] = set()
    low_cost: set[str] = set()

    for subject_code, _display in SUBJECTS:
        try:
            subject_ztc = fetch_class_type_numbers(session, term_id, subject_code, "OER")
            ztc.update(subject_ztc)
            print(f"{term_label}: {subject_code}: {len(subject_ztc)} ZTC sections")
        except Exception as exc:  # noqa: BLE001
            print(f"Warning: {term_label} {subject_code} ZTC lookup failed: {exc}", file=sys.stderr)

        try:
            subject_low = fetch_class_type_numbers(session, term_id, subject_code, "LCT")
            low_cost.update(subject_low)
            print(f"{term_label}: {subject_code}: {len(subject_low)} low-cost sections")
        except Exception as exc:  # noqa: BLE001
            print(f"Warning: {term_label} {subject_code} low-cost lookup failed: {exc}", file=sys.stderr)

        time.sleep(0.15)

    for class_number, section in sections.items():
        flags = set(section.get("materials") or [])
        # Remove any earlier experimental OER label. In the LACCD public search,
        # backend value OER means the student-facing ZTC designation.
        flags.discard("OER")
        if class_number in ztc:
            flags.add("ZTC")
        if class_number in low_cost:
            flags.add("Low Cost")
        ordered = [label for label in ("ZTC", "Low Cost") if label in flags]
        section["materials"] = ordered
        section["materials_text"] = "; ".join(ordered)
...
def main() -> int:
    ...
    for term in terms:
        ...
        # ZTC and Low Textbook Cost are Class Search Type filters in LACCD SIS,
        # not reliable text inside each result row. Query those filters
        # separately and join their class numbers back to the main feed.
        enrich_material_flags(
            session,
            str(term["id"]),
            str(term["label"]),
            term_sections,
